...
function useAbility(s:GameState,e:Entity):boolean{
 ...
 let affected=false;for(const ally of s.entities)if(ally.side===e.side&&alive(ally)&&ally.kind==='unit'&&!ally.illusion&&distance(e,ally)<5){ally.hp=Math.min(ally.maxHp,ally.hp+25);ally.surgeUntil=s.time+6;affected=true;}if(!affected)return false;runtime(s).abilities.set(e.id,s.time+24);
...
function damage(s:GameState,a:Entity,b:Entity):void{
 ...
 const base=d?.damage??19;const bonus=d?.ability==='momentum'?1+a.momentum*.65:emplaced(s,a)?1.25:1;const hit=Math.max(1,base*bonus*(b.kind==='building'?(d?.buildingDamageMultiplier??1):1)-armor)*(a.illusion?.25:1);
 a.cooldown=(d?.cooldown??1.4)/(d?.ability==='momentum'?1+a.momentum*.25:1);if(d?.ability==='momentum')a.momentum=Math.min(1,a.momentum+.15);a.animation='attack';a.animTime=0;const event=emit(s,'attack',a,b.id);runtime(s).hits.push({source:a,target:b,amount:hit,event});
}
...
export function stepGame(s:GameState,dt:number):void{
 ...
 if(o.type==='hold'){const b=enemy(s,e,weaponRange(s,e),true);if(b&&near(s,e,b,weaponRange(s,e))){fight(s,e,b,dt);if(!e.illusion&&(d.ability==='illusion'||d.ability==='heal'||d.ability==='surge'))useAbility(s,e);}continue;}
 ...
 const b=enemy(s,e,d.role==='worker'?2:Math.min(d.sight,7));if(b){fight(s,e,b,dt);if(!e.illusion&&(d.ability==='illusion'||d.ability==='heal'||d.ability==='surge'))useAbility(s,e);}else if(o.type==='attackMove'&&move(s,e,o,dt,.65))assign(s,e,{type:'idle'});
 }
 resolveHits(s);
