...
export function route(s:GameState,from:Vec,to:Vec,reach:number):Vec[]{
 const coarse=routeOnGrid(s,from,to,reach,.5);
 return coarse.length?coarse:routeOnGrid(s,from,to,reach,.25);
}
// Refine only failed searches: legal one-tile building gaps can fall between coarse centers.
function routeOnGrid(s:GameState,from:Vec,to:Vec,reach:number,CELL:number):Vec[]{
