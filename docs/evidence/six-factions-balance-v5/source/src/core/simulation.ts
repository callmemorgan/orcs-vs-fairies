...
function spawn(s:GameState,side:Side,kind:Entity['kind'],role:UnitRole|BuildingRole,x:number,y:number,progress=1):Entity{
 ...
 const e:Entity={id:s.nextId++,side,kind,role,x,y,hp:progress===1?def.hp:Math.max(1,def.hp*.1),maxHp:def.hp,order:{type:'idle'},cooldown:0,progress,queue:[],trainProgress:0,facing:2,animation:'idle',animTime:0,momentum:0,illusion:false,expires:0,carried:0,carriedKind:'wood',path:[]};if(kind==='unit'&&f.id==='automata'){e.maxShield=({worker:12,melee:45,ranged:35,special:45} as Record<string,number>)[role];e.shield=e.maxShield;}s.entities.push(e);return e;
}
...
function damage(s:GameState,a:Entity,b:Entity):void{
 const d=a.kind==='unit'?unitDef(s,a):null;const armor=(b.kind==='unit'?unitDef(s,b).armor:3)+(emplaced(s,b)?2:0);
