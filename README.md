...
A browser RTS with six playable factions, seeded maps, three resources and a terminal interface for game-playing agents. Build a settlement, gather resources, recruit an army and destroy every completed enemy stronghold. Any faction can face any other, including itself.
...
The game uses Phaser, TypeScript and Vite. Its simulation runs independently of the renderer. All factions have seven unit types and six building types, with Blender-rendered models, eight-direction unit animations and selection artwork. ImageGen references and editable Blender sources are included.
...
Choose your faction, AI opponent, map size and seed in the menu. Small maps are 36 × 36 tiles, medium 48 × 48 and large 64 × 64, and huge 88 × 88. A seed reproduces the terrain and resource layout. Starting locations and resource placement are rotationally symmetric.
...
Construction and recruitment appear as illustrated commands when a worker or production building is selected. Z/C/B/V/N/M trigger the displayed construction or recruitment slots. Build, Recruit and Orders tabs keep mixed-selection commands within the panel. Hover or focus a command for its description, cost, shortcut and unavailable reason. The selection bar shows portraits, health, construction and recruitment progress. Click a recruitment queue portrait to cancel that entry for a full resource refund. Canceling the active entry resets progress for the next unit; canceling a waiting entry leaves active progress intact. Click a roster portrait to select that unit, or Shift-click to remove it. Saved groups also have clickable buttons.

Open Technologies to see the three-age technology tree and research at an available building. Harvest Drills gathers 30% faster and Courier Training moves 20% faster. Town Age unlocks faction specialists, cavalry, fortifications and expansion headquarters; Citadel Age unlocks siege. Barracks research melee weapon and armor upgrades. Research runs alongside recruitment and costs are paid up front. See [three-age progression](docs/THREE_AGES.md) for costs, counters and verification.
...
See [the current three-age verification](docs/THREE_AGES.md) and [remaining limitations](docs/KNOWN_LIMITATIONS.md). The [expansion report](docs/EXPANSION_DAY.md) and [six-faction ladder](docs/evidence/six-factions-final-v6/REPORT.md) describe the earlier build. The original [64-game ladder](docs/evidence/ladder-64/REPORT.md) remains unchanged. Earlier four-faction and two-faction evidence is historical.
