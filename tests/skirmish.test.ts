...
const coverage=new Map<string,{hits:Set<string>;buildings:Set<string>}>();
it.each(factions.flatMap(f=>factions.map(o=>[f,o] as const)))('completes a real %s vs %s AI skirmish', (faction,opponent) => {
  ...
  const report={faction,opponent,seed:4127,seconds:s.time,winner:s.winner,draw:s.draw,hq:s.entities.filter(e=>e.role==='hq').map(e=>({side:e.side,hp:e.hp})),trained,deposited,attacks,hitsByRole,abilities,emplacementHits,raisedHits,roles:roles.map(r=>[...r]),built:built.map(b=>[...b])};
  mkdirSync('work/day-expansion/regression-matches',{recursive:true});writeFileSync(`work/day-expansion/regression-matches/${faction}-${opponent}.json`,JSON.stringify(report,null,2));
  if(!isGameOver(s))writeFileSync(`work/day-expansion/regression-matches/${faction}-${opponent}-unfinished.json`,JSON.stringify({time:s.time,players:s.players,entities:s.entities},null,2));
  ...
  for (const side of [0, 1]) {
    ...
    const f=s.players[side].faction,c=coverage.get(f)??{hits:new Set<string>(),buildings:new Set<string>()};
    for(const [role,hits] of Object.entries(hitsByRole[side]))if(hits)c.hits.add(role);
    for(const role of built[side])c.buildings.add(role);coverage.set(f,c);
...
afterAll(()=>{
 for(const faction of factions){const c=coverage.get(faction)!;for(const role of ['melee','ranged','special'])expect(c.hits.has(role),`${faction} ${role} never fought`).toBe(true);expect(c.buildings).toEqual(new Set(['hq','barracks','depot','tower']));}
});
