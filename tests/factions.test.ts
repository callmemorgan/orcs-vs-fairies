...
describe('six factions',()=>{
 ...
 it('has unique content IDs and complete rosters',()=>{const all=Object.values(FACTIONS).flatMap(f=>[...Object.values(f.units),...Object.values(f.buildings)]);expect(new Set(all.map(d=>d.id)).size).toBe(78);for(const d of all){expect(d.description.length).toBeGreaterThan(15);expect(d.hp).toBeGreaterThan(0);}});
