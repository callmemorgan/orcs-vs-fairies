...
import { canPlace, issueCommand, stepGame } from '../core/simulation';
...
export default class GameScene extends Phaser.Scene {
  ...
  public get artStatus(){return {enabled:this.art?.enabled??false,loaded:this.art?.loaded??false,assets:this.art?.assetCount??0,renderedUnits:this.art?.renderedUnits??0};}
  ...
  preload(){this.art=new ArtRuntime(this,new URLSearchParams(location.search).get('art')!=='placeholder');this.art.preload();}
  create() {
    ...
    this.drawGround();
    this.cameras.main.setBounds(-40,-80,3280,1850).setZoom(1);
    this.centerOn(8,8);
    ...
    this.input.on('pointerdown',(p:Phaser.Input.Pointer)=>{
      ...
      if(this.paused||this.state.winner!==null)return;
  ...
  public restart(state:GameState){this.audio?.reset();this.resultSoundPlayed=false;this.art.reset();this.state=state;this.paused=false;this.accumulated=0;this.attackedNoticeAt.clear();this.buildingAlertAt=-Infinity;this.workerAlertAt=-Infinity;this.groups={};this.markers=[];this.setBuildRole(null);this.select([]);this.drawGround();this.drawFog();this.centerOn(8,8);}
  public holdPosition(){if(this.paused||this.state.winner!==null||!issueCommand(this.state,0,{type:'hold',ids:this.selected}))return false;this.attackMode=false;this.setBuildRole(null);this.audio?.play('order');this.options.onNotice('Holding position: attack in range without pursuing.');return true;}
  ...
  private key(e:KeyboardEvent){
    ...
    if((this.paused||this.state.winner!==null)&&['KeyA','KeyQ','KeyF','KeyX','KeyH'].includes(e.code))return;
  ...
  private order(p:Phaser.Input.Pointer,attack=false){
    ...
    if(hit&&hit.side===1)ok=issueCommand(this.state,0,{type:'attack',ids:this.selected,target:hit.id});
      ...
      else ok=issueCommand(this.state,0,{type:attack?'attackMove':'move',ids:this.selected,x:Phaser.Math.Clamp(pos.x,.5,47.5),y:Phaser.Math.Clamp(pos.y,.5,47.5)});
    }
  ...
  private processEvents(){
    ...
    if(state.winner!==null&&!this.resultSoundPlayed){this.resultSoundPlayed=true;this.audio?.play(state.winner===0?'victory':'defeat');}
    if(buildingAlert){
  ...
  private drawGround(){
    ...
    for(let y=0;y<this.state.height;y++)for(let x=0;x<this.state.width;x++){
      ...
      const shades=path?[0x7c7850,0x85805a,0x77754e]:[0x536b48,0x596f49,0x60774e,0x5a7049,0x4f6744];
      this.diamond(g,p.x,p.y,65,33,shades[n%shades.length]);
      if(!path&&n%4===0){g.lineStyle(1,0x91a262,.35);g.lineBetween(p.x-8,p.y+3,p.x-6,p.y-1);g.lineBetween(p.x-4,p.y+5,p.x-3,p.y+1);}
      if(Math.hypot(x-23.5,y-23.5)<3)this.diamond(g,p.x,p.y,59,28,0x858371);
    }
  }
  private drawActors(){
    const g=this.actors;g.clear();this.art.begin();
    const objects=[...this.state.resources.filter(r=>r.amount>0).map(r=>({sort:r.x+r.y,r})),...this.state.entities.filter(e=>this.visible(e)).map(e=>({sort:e.x+e.y,e}))].sort((a,b)=>a.sort-b.sort);
    for(const obj of objects){
      if('r' in obj){const r=obj.r,p=project(r.x,r.y);if(!this.state.explored[0].has(Math.floor(r.y)*48+Math.floor(r.x)))continue;
        const environment=r.kind==='ore'?'ore':r.id%3===0?'tree-oak':'tree-pine';
    ...
    for(const [i,x,y,id] of [[0,22.5,22.5,'ruin-ring'],[1,20.5,23.5,'ruin-pillar'],[2,26.5,24.5,'ruin-pillar']] as const){
      if(this.state.explored[0].has(Math.floor(y)*48+Math.floor(x))){const p=project(x,y);this.art.environment(`ruin:${i}`,id,p.x,p.y);}
    }
    for(const r of this.state.resources)if(r.kind==='wood'&&r.amount<=0&&this.state.explored[0].has(Math.floor(r.y)*48+Math.floor(r.x))){const p=project(r.x,r.y);this.art.environment(`resource:${r.id}`,'stump',p.x,p.y);}
    this.art.end();
  }
  private drawFog(){const g=this.fog;g.clear();for(let y=0;y<48;y++)for(let x=0;x<48;x++){const i=y*48+x;if(this.state.visible[0].has(i))continue;const p=project(x+.5,y+.5);this.diamond(g,p.x,p.y,65,33,0x102022,this.state.explored[0].has(i)?.48:.98);}}
  private drawOverlay(){
