...
export function mountShell(root:HTMLElement,onStart:(faction:FactionId,opponent:FactionId,mapSize:MapSize,seed:number)=>void) {
  ...
  return {
    ...
    update:(s:GameState,selected:number[],cb:HudCallbacks)=>{
      ...
      el('#portrait').dataset.tooltip=entityDef?`<h3>${escape(entityDef.name)}</h3><p>${escape(entityDef.description)}</p>`:'<h3>Select a unit</h3><p>Click a unit or drag across your army. Shift adds to your selection. F2 selects combat units.</p>';
      ...
      const key=`${actionMode}:${player.faction}:${own.map(e=>`${e.id}/${e.progress>=1}`).join(',')}:${abilityIds.join(',')}`;
      if(key!==actionsKey){
      ...
      if(queue.dataset.key!==queueKey){queue.dataset.key=queueKey;queue.innerHTML=producers.map(producer=>`<div class="queue-row" data-producer="${producer.id}"><span class="queue-label">RECRUITING</span>${producer.queue.map((role,i)=>`<div class="queue-item" tabindex="0" data-tooltip="${escape(`<h3>${definition.units[role].name}</h3><p>${i===0?'In production':'Queued'} • ${definition.units[role].trainTime}s recruitment</p>`)}"><img src="${art(definition.units[role].id)}" alt="${escape(definition.units[role].name)}"/>${i===0?'<b></b><span class="queue-progress"><i></i></span>':`<em>${i+1}</em>`}</div>`).join('')}</div>`).join('');}
