...
export type Command = {type:'move'|'attackMove';ids:number[];x:number;y:number} | {type:'attack'|'gather'|'repair';ids:number[];target:number} | {type:'build';ids:number[];role:BuildingRole;x:number;y:number} | {type:'train';id:number;role:UnitRole} | {type:'cancelTrain';id:number;index:number} | {type:'stop'|'hold';ids:number[]} | {type:'ability';ids:number[]} | {type:'setRally';ids:number[];x:number;y:number} | {type:'clearRally';ids:number[]};
